...
def calculate(lmp, kkflag, ngrid, nbox, rcutfac, twojmax, switch, radelem, sigma): 
    ...
    compute ggrid all gaussian/grid/local  grid {ngrid} {ngrid} {ngrid}  {rcutfac}  {radelem}  {sigma}
    ...
    test_grid_arr = extract_compute_np_fs(lmp, "ggrid", lammps_constants.LMP_STYLE_LOCAL, 2)
...
# nbox_lst = [1,2,4,8,10] 
ngrid_lst = [5, 10, 20, 50, 70, 90, 100, 300, 500]
...
for ngrid in ngrid_lst:
    ...
    comptime = comptime_reduced[0]/size
...
np.savetxt(f"metrics_{nbox}_kk{kkflag}.dat", dat)
